...
class report_data():
    ...
    def one_angle(data, cols, time, to_time, kh_time, fc_time, bc_time,pel_initial_time):
        ang = {
            'to_time'  : {},
            'kh_time'  : {},
            'fc_time'  : {},
        ...
        for col in cols:
            ...
            ang['max'][col] = round(data[col].iloc[int(fc_time):int(bc_time+1)].max(),2)
            ang['min'][col] = round(data[col].iloc[int(fc_time):int(bc_time+1)].min(),2)
            ang['max_time'][col] = np.where(data[col] == data[col].iloc[int(fc_time):int(bc_time+1)].max())[0][0]
            ang['min_time'][col] = np.where(data[col] == data[col].iloc[int(fc_time):int(bc_time+1)].min())[0][0]
            ...
            if col in ['pelvis_rotational_velocity_with_respect_to_ground']:
                ...
                plt.axvline(time[pel_initial_time], color = 'firebrick', linestyle = '--',alpha=0.7)
